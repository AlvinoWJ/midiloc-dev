...
export default function LocationPickerMap({ onConfirm }: LocationPickerMapProps) {
  const defaultCenter: [number, number] = [-6.20, 106.81];
  const [position, setPosition] = useState<LatLng>(new LatLng(defaultCenter[0], defaultCenter[1]));
  ...
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (location) => {
          const userLatLng = new LatLng(location.coords.latitude, location.coords.longitude);
