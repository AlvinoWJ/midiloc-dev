...
/**
 * @route PATCH /api/progress/[id]/izin_tetangga/approval
 * @description Melakukan Approval (Selesai/Batal) pada Izin Tetangga.
 */
export async function PATCH(
  ...
  try {
    ...
    const parsed = ITApprovalSchema.safeParse(body);
    if (!parsed.success)
      return NextResponse.json(
    ...
    // 4. Execute RPC
    const { data, error } = await supabase.rpc("fn_it_approve", {
      p_user_id: user.id,
      p_branch_id: user.branch_id,
      p_progress_kplt_id: params.id,
      p_final_status: parsed.data.final_status_it, // "selesai" | "batal"
    });
    ...
    if (error) {
      ...
      if (msg.includes("required fields are missing"))
        return NextResponse.json(
          { error: "Incomplete data", message: msg },
          { status: 422 }
        );
      ...
      console.error("[IT_APPROVAL_RPC]", error);
      return NextResponse.json(
    ...
    console.error("[IT_APPROVAL_PATCH_UNHANDLED]", err);
    return NextResponse.json(
      { error: "Internal Server Error" },
      { status: 500 }
    );
