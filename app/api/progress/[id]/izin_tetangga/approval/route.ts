...
export async function PATCH(
  ...
  if (error) {
    ...
    const isMissing = msg.includes("required fields are missing");
    return NextResponse.json(
      {
        error: isAlready
          ? "Izin Tetangga already finalized"
          : isInvalid
          ? "Invalid status"
          : isMissing
          ? "Required fields missing"
          : "Failed to approve Izin Tetangga",
        detail: supabaseError?.message ?? error,
      },
      { status: isAlready ? 409 : isInvalid ? 422 : isMissing ? 422 : 500 }
    );
  }
